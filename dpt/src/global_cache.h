...
struct global_cache_t {
    ...
    // [sholl] modified the function according to the new nanoflann version
    inline bool query(const std::vector<Float> &pss, std::vector<Float>& v1, std::vector<Float>& v2)
    {
        if (!is_ready)
            return false;
        const Float radius =
            dim * (PSS_QUERY_DIST * PSS_QUERY_DIST);  // nanoflann uses squared L2 distance
        std::vector<ResultItem<std::uint32_t, Float>> ret_matches;
        SearchParameters searchParameters;
        const size_t nMatches = data_tree->radiusSearch(&pss[0], radius, ret_matches, searchParameters);
        if (!nMatches)
            return false;
        double sum_w = 0;
