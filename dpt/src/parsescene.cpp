...
std::shared_ptr<DptOptions> ParseDptOptions(pugi::xml_node node) {
    ...
    for (auto child : node.children()) {
        ...
        // begin [sholl]
        else if (name == "compute_mse")
            dptOptions->compute_mse = child.attribute("value").value() == std::string("true");
        else if (name == "direct_depth_max")
            dptOptions->direct_depth_max = std::stoi(child.attribute("value").value());
        else if (name == "expected_lifetime")
        {
            auto const expected_lifetime = to_float<Float>(child.attribute("value").value());
            if (expected_lifetime.value_or(0))
                dptOptions->expected_lifetime = *expected_lifetime;
            else
                std::cerr << "invalid value for expected_lifetime; defaulted to " << dptOptions->expected_lifetime << std::endl;
        }
        else if (name == "chain_count")
            dptOptions->chain_count = std::stoi(child.attribute("value").value());
        else if (name == "realization_count")
        {
            int const realization_count = std::stoi(child.attribute("value").value());
            if (realization_count >= 1)
                dptOptions->realization_count = static_cast<std::size_t>(realization_count);
            else
                std::cerr << "realization_count < 1; defaulted to 1" << name << std::endl;
        }
        else if (name == "rao_blackwellize")
            dptOptions->rao_blackwellize = child.attribute("value").value() == std::string("true");
        else if (name == "reference")
            dptOptions->reference = child.attribute("value").value();
        else if (name == "sigma")
        {
            auto const sigma = to_float<Float>(child.attribute("value").value());
            if (sigma.value_or(0) > 0)
                dptOptions->sigma = *sigma;
            else
                std::cerr << "invalid value for sigma; defaulted to " << dptOptions->expected_lifetime << std::endl;
        }
        else if (name == "chain_count")
        {
            int const chain_count = std::stoi(child.attribute("value").value());
            if (chain_count >= 1)
                dptOptions->chain_count = static_cast<std::size_t>(chain_count);
            else
                std::cerr << "chain_count < 1; defaulted to 1" << chain_count << std::endl;
        }
        else if (name == "fps")
        {
            int const fps = std::stoi(child.attribute("value").value());
            if (fps >= 1)
                dptOptions->frame_period = std::chrono::duration<Float>(Float{ 1 } / fps);
            else
                std::cerr << "fps < 1; defaulted to 1" << std::endl;
        }
        else if (name == "rendering_time")
        {
            auto const rendering_time = to_float<Float>(child.attribute("value").value());
            if (rendering_time.value_or(0) > 0)
                dptOptions->rendering_time = std::chrono::duration<Float>(*rendering_time);
            else
                std::cerr << "rendering_time < 0s; defaulted to 1s" << std::endl;
        }
        // end [sholl]
        else {
    ...
    // begin [sholl]
    if (dptOptions->direct_depth_max < dptOptions->minDepth)
        dptOptions->direct_depth_max = dptOptions->minDepth - 1;

    dptOptions->direct_depth_min = dptOptions->minDepth;
    dptOptions->minDepth = dptOptions->direct_depth_max + 1;
    // end [sholl]
