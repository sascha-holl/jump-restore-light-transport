...
// [sholl] modified the function according to the new embree version
    ...
    ShapeInst& shapeInst)
{
    ...
    RTCRayQueryContext rtcRayQueryContext;
    rtcInitRayQueryContext(&rtcRayQueryContext);

    RTCIntersectArguments rtcIntersectArguments;
    rtcInitIntersectArguments(&rtcIntersectArguments);
    rtcIntersectArguments.context = &rtcRayQueryContext;

    rtcIntersect1(scene->rtcScene, &rtcRay, &rtcIntersectArguments);
...
bool Occluded(const Scene* scene, const Float time, const RaySegment& raySeg)
{
    ...
    RTCOccludedArguments rtcOccludedArguments;
    rtcInitOccludedArguments(&rtcOccludedArguments);
    rtcOccludedArguments.context = &rtcRayQueryContext;

    rtcOccluded1(scene->rtcScene, &rtcRay, &rtcOccludedArguments);
