...
void SkipToIdx(std::fstream &fs, const short version, const size_t idx) {
    ...
    fs.seekg(-static_cast<std::streamoff>(sizeof uint32_t), fs.end); // [sholl] fixed error: unary minus operator applied to unsigned type, result still unsigned
    uint32_t count = 0;
    fs.read((char *)&count, sizeof(uint32_t));
    size_t offset = 0;
    if (version == MTS_FILEFORMAT_VERSION_V4) {
        fs.seekg(-static_cast<std::streamoff>(sizeof uint64_t) * (count - idx) - sizeof(uint32_t), fs.end); // [sholl] fixed error: unary minus operator applied to unsigned type, result still unsigned
        fs.read((char *)&offset, sizeof(size_t));
    } else {  // V3
        fs.seekg(-static_cast<std::streamoff>(sizeof uint32_t) * (count - idx + 1), fs.end); // [sholl] fixed error: unary minus operator applied to unsigned type, result still unsigned
        uint32_t upos = 0;
