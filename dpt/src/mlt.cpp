...
void MLT(const Scene *scene, const std::shared_ptr<const PathFuncLib> pathFuncLib) {
    ...
    DirectLighting(scene, directBuffer); // [sholl]
    ...
    std::cout << "m_min_chain_sample_count = " << numSamplesPerChain + 1 << '\n'
        << "last_chain_sample_count = " << numSamplesPerChain << '\n'
        << "chain_count = " << numChains << '\n';
    ...
    ParallelFor([&](const int chainId) {
        ...
        for (int sampleIdx = 0; sampleIdx < numSamplesThisChain; sampleIdx++) {
            ...
            if (a > Float(0.0) && uniDist(rng) <= a) {
                ...
                //        std::cout << chainId << ", " << "adjacentReject = " << adjacentReject << '\n'; // [sholl]
    ...
    //system(hdr2ldr.c_str());
    WriteImage(scene->outputName + ".exr", GetFilm(scene->camera.get()).get());
    // end [sholl]
