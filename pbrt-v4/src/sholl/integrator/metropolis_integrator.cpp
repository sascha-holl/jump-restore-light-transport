...
#include "metropolis_integrator.hpp"
...
namespace pbrt
{
	...
	SampledSpectrum metropolis_integrator::sample_path(std::size_t const s, std::size_t const t, local_exploration_sampler& sampler,
	...
	SampledSpectrum metropolis_integrator::sample_debug_path(Point2f& raster_point, std::size_t const s, std::size_t const t, local_exploration_sampler& sampler,
		ScratchBuffer& scratch_buffer, SampledWavelengths& lambda) const
	{
		lambda = m_camera.GetFilm().SampleWavelengths(sampler.Get1D());
	...
	SampledSpectrum metropolis_integrator::sample_path(std::size_t const depth, local_exploration_sampler& sampler,
		ScratchBuffer& scratch_buffer, Point2f& raster_point, SampledWavelengths& lambda) const
	{
		std::size_t s, t,
			strategy_count;
	...
	SampledSpectrum metropolis_integrator::sample_debug_path(Point2f& raster_point, std::size_t const depth, local_exploration_sampler& sampler,
		ScratchBuffer& scratch_buffer, SampledWavelengths& lambda) const
	{
		std::size_t s, t,
			strategy_count;
		...
		auto const contribution = sample_debug_path(raster_point, s, t, sampler, scratch_buffer, lambda);
		return strategy_count * contribution;
	}
} // namespace pbrt
