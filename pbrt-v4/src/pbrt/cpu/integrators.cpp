...
SampledSpectrum ConnectBDPT(const Integrator &integrator, SampledWavelengths &lambda,
                            ...
                            Float *misWeightPtr = nullptr, bool apply_multiple_importance_sampling_weighting = true /* [sholl] */);
...
SampledSpectrum ConnectBDPT(const Integrator &integrator, SampledWavelengths &lambda,
                            Vertex *lightVertices, Vertex *cameraVertices, int s, int t,
                            LightSampler lightSampler, Camera camera, Sampler sampler,
                            pstd::optional<Point2f> *pRaster, Float *misWeightPtr, bool apply_multiple_importance_sampling_weighting)
{
    SampledSpectrum L(0.f);
    ...
    if (apply_multiple_importance_sampling_weighting)
        L *= misWeight;
    // end [sholl]
...
void MLTIntegrator::Render() {
    ...
    auto const rendering_time = std::chrono::duration_cast<std::chrono::seconds>(end - begin).count();
...
std::unique_ptr<Integrator> Integrator::Create(
    ...
    // begin [sholl]
        ...
        integrator = metropolis_integrator::Create(parameters, aggregate, lights, camera, loc);
    else if (name == "metropolis_restore")
        integrator = metropolis_restore_integrator::Create(parameters, aggregate, lights, camera, loc);
    // end [sholl]
    else
...
// [sholl]
#include <sholl/integrator/metropolis_integrator.cpp>
#include <sholl/integrator/metropolis_restore_integrator.cpp>
