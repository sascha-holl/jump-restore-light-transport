...
{
    // [sholl] replaced .clear with .resize(0)
    path.camSurfaceVertex.resize(0);
    path.lgtSurfaceVertex.resize(0);
...
{
    // [sholl]
    return (camDepth + lightDepth - 1) * 2 + 1;
...
{
    // [sholl] shortened function naming to prevent too long command line
    std::string baseName =
        "p" + std::to_string(camDepth) + "_" + std::to_string(lightDepth);
...
std::string GetFuncNameBidir(const int camDepth, const int lightDepth, const PathFuncMode mode) 
{
    // [sholl] shortened function naming to prevent too long command line
    std::string baseName =
        "b" + std::to_string(camDepth) + "_" + std::to_string(lightDepth);
    if (mode == PathFuncMode::Full) {
        return baseName + "_full";
    } else if (mode == PathFuncMode::Static) {
        return baseName + "s";
...
std::string GetFuncNameBidirMALA(const int camDepth, const int lightDepth, const PathFuncMode mode) 
{
    // [sholl] shortened function naming to prevent too long command line
    std::string baseName =
        "bm" + std::to_string(camDepth) + "_" + std::to_string(lightDepth);
    if (mode == PathFuncMode::Full) {
        return baseName + "_full";
    } else if (mode == PathFuncMode::Static) {
        return baseName + "s";
